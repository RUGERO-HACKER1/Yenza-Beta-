...
const OpportunityDetailsPage = () => {
    ...
    useEffect(() => {
        ...
        // Fetch Opportunity
        fetch(`http://localhost:5000/opportunities/${id}`)
            .then(res => {
            ...
            .then(data => {
                ...
                if (data.companyId && data.companyId !== 'admin-manual') {
                    fetch(`http://localhost:5000/companies/${data.companyId}`)
                        .then(res => res.json())
                        .then(comp => {
        ...
        if (user && user.role === 'user') {
            fetch(`http://localhost:5000/applications?userId=${user.id}&opportunityId=${id}`)
                .then(res => res.json())
                .then(data => {
    ...
    const renderExternalApplyModal = () => {
        ...
        const handleContinue = () => {
            ...
            fetch(`http://localhost:5000/opportunities/${op.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
