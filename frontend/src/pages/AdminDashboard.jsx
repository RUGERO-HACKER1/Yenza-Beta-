...
const AdminDashboard = () => {
    ...
    const handleCompanyAction = async (id, action) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL}/admin/companies/${id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                                            ...
                                            {companies.sort((a, b) => (a.check === 'pending' ? -1 : 1)).map(comp => (
                                                <tr key={comp.id} style={{ borderBottom: '1px solid var(--border)' }}>
                                                    ...
                                                    <td style={{ padding: '1rem', fontSize: '0.9rem' }}>
                                                        ...
                                                        {comp.documentUrl && (
                                                            <a
                                                                href={`${import.meta.env.VITE_API_URL}${comp.documentUrl}`}
                                                                target="_blank"
                                                        ...
                                                        {op.type === 'gig' && op.status === 'in_progress' && (
                                                            <button
                                                                onClick={async () => {
                                                                    ...
                                                                    await fetch(`${import.meta.env.VITE_API_URL}/admin/opportunities/${op.id}`, {
                                                                        method: 'PATCH',
                                                                        headers: { 'Content-Type': 'application/json' },
